    ...
    <div className="space-y-6">
      ...
      {!hasResult && (
        <div className="rounded-lg border border-zinc-200 bg-white p-6 space-y-5">
          ...
          {/* Segment selector */}
          <div>
            <label className="block text-sm font-medium text-zinc-700 mb-1" htmlFor="group-select">
              Toevoegen aan segment <span className="text-zinc-400 font-normal">(optioneel)</span>
            </label>
            <select
              id="group-select"
              value={selectedGroupId}
              onChange={(e) => setSelectedGroupId(e.target.value)}
              className="block w-full rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm focus:outline-none focus:border-zinc-500"
            >
            ...
            {selectedGroup && (
              <p className="mt-1.5 flex items-center gap-1.5 text-xs text-zinc-500">
                <span className="inline-block h-2 w-2 rounded-full" style={{ backgroundColor: selectedGroup.color }} />
                Alle geïmporteerde contacten worden toegevoegd aan <strong>{selectedGroup.name}</strong>
              </p>
            )}
          </div>
        </div>
            ...
            <div>
              ...
              {batches > 1 && (
                <p className="text-xs text-zinc-500 mt-0.5">
                  Wordt verwerkt in {batches} batches van {BATCH} contacten
                </p>
              )}
            </div>
      ...
      {rows.length > 0 && !hasResult && (
        <form action={formAction}>
          ...
          {selectedGroupId && <input type="hidden" name="group_id" value={selectedGroupId} />}
          ...
          <div className="rounded-lg border border-amber-200 bg-amber-50 px-5 py-4 text-sm text-amber-800 mb-4">
            ...
            {selectedGroup && <> Ze worden ook toegevoegd aan <strong>{selectedGroup.name}</strong>.</>}
          ...
          <div className="rounded-lg border border-zinc-200 bg-white p-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="rounded-lg bg-green-50 border border-green-200 px-4 py-4 text-center">
                ...
                <p className="text-xs text-green-600 mt-1">Nieuw geïmporteerd</p>
              </div>
            ...
            {state.group_name && state.group_added !== undefined && (
              <div className="mt-4 rounded-md border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800">
                <strong>{state.group_added}</strong> contacten toegevoegd aan segment <strong>{state.group_name}</strong>
                {state.skipped > 0 && ' (inclusief reeds bestaande contacten uit de CSV)'}
              </div>
            )}
